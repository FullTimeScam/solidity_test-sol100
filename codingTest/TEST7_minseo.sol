...
function charge() public payable {
    // 돈을 받는 주체가 없어서
    require((msg.value == 1 ether || address(this).balance >= 1 ether) && myCar.status == CarStatus.off);
    // 0주소에게 돈이 가게끔만 짜봅시다.
    // payable(address(0)).transfer(1 ether); // transfer 활용
    (bool success, ) = address(0).call{value: 1 ether}(""); // call 활용
    require(success);
